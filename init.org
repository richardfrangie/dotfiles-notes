...
#+begin_src emacs-lisp
  ...
  ;; To use the :ensure-system-package keyword install this extension:
  ...
  ;; To use the use-package-ensure-system-package install this package:
  (use-package system-packages)


  ;; Built-in packages can be upgraded via a package archive.
...
#+begin_src emacs-lisp
  ...
  (setq hi-lock-face-defaults
        '("hi-aquamarine" "hi-salmon" "hi-pink" "hi-green" "hi-blue"
          "hi-yellow" "hi-black-b" "hi-blue-b" "hi-red-b" "hi-green-b"
          "hi-black-hb"))
  ;; Change the highlight color to match the background theme
  (custom-set-faces '(hi-yellow
...
#+begin_src emacs-lisp
  ...
  ;; Wait 0.5 seconds after Emacs opens, then position the frame
  (add-hook 'window-setup-hook
            (lambda ()
              (run-at-time 0.5 nil
                           #'rf-position-and-resize-frame-bottom-right)))
  ;; or
...
#+begin_src emacs-lisp
  ...
  (defun xah-mouse-click-to-search (Click)
    "Mouse click to start `isearch-forward-symbol-at-point'
  (emacs 24.4) at clicked point.
...
#+begin_src emacs-lisp
  ...
  ;; Trick: set next/previous history in case the vertico-map is unavailable.
  (keymap-set minibuffer-local-map "C-S-C" 'previous-line-or-history-element)
  (keymap-set minibuffer-local-map "C-S-T" 'next-line-or-history-element)
...
#+begin_src emacs-lisp
  ...
  Modified version that fixes the error that occurred when reaching
  the last line of the buffer"
    (interactive)
    (save-excursion
      (if (and n (not (line-move-visual n t)))
          (end-of-visual-line)
        (beginning-of-visual-line))
      (point)))
...
#+begin_src emacs-lisp
  ...
  With the universal-command, starting in the root directory."
    ...
    (defvar-keymap consult-search-map
      :doc "Keymap for the consult search and select commands."
      ...
      "{" #'consult-register)
    (keymap-set global-map "C-." consult-search-map))
...
#+begin_src emacs-lisp
  ...
  (use-package embark
    ...
    (add-to-list 'embark-repeat-actions 'rf-org-down-heading)
    ...
    (defun re-embark-kill-buffer (&optional arg)
      ...
      (if-let ((targets (embark--targets)))
          (let* ((target
            ...
            (cond
             ;; MODIFICATION: don't delete the scratch, Messages,... buffer
             ;; Also replace in the `xah-close-current-buffer' function
             ((or (member buffer '("*scratch*" "*Messages*"
                                   "*gptel*" "*Gemini*" "*eshell*"))
                  (string-match-p
                   ".* Agent @ .*" buffer))
...
#+begin_src emacs-lisp
  ...
  (defvar-keymap mark-next-repeat-map
    :doc "Keymap to repeat next mark."
    :repeat t
    "d" #'mc/mark-next-like-this)

  (defvar-keymap mark-previous-repeat-map
    :doc "Keymap to repeat previous mark."
    :repeat t
...
*** Control shift
Manage repeat commands  and maps triggered by the =Control-Shift= key.
...
#+begin_src emacs-lisp
  ...
  (keymap-set global-map "C-S-u" switch-resize-windows-repeat-map)
  ...
  (keymap-set global-map "C-S-j" rectangle-related-map)
...
*** Misc
Manage repeat commands  and maps triggered by the minor modes like ~prog-mode-map~.
...
#+begin_src emacs-lisp
  ...
  (use-package dired
    ...
    (defun rf-switch-to-image-dired-screenshots ()
      "Switch to the `~/Pictures/Screenshots/' image dired directory"
      (interactive)
      (image-dired "~/Pictures/Screenshots/")))
...
#+begin_src emacs-lisp
  ...
  (defvar rf-main-test-directory "/home/rich/projects/tests/TEST/"
...
#+begin_src emacs-lisp
  ...
  (defun rf-org-bindings-setup ()
    ...
    (defvar-keymap org-link-preview-repeat-map
      :doc "Keymap to repeat `org-link-preview'.  Used in `repeat-mode'."
      :repeat t
      "i" #'org-link-preview)
...
#+begin_src emacs-lisp
  ...
  (defun rf-org-babel-setup ()
    ...
    ;; Explicitly load the gnuplot Babel library(to use gnuplot on org blocks)
    (require 'ob-gnuplot)
...
#+begin_src emacs-lisp
  ...
  (use-package markdown-mode
    ...
    (set-face-attribute 'markdown-header-delimiter-face nil
                        :foreground "#5B6268")
    ;; Make Markdown headers inherit from Org header colors
    ...
    ;; Qwen3.7-Plus:
    (defun qw-outline-down-heading ()
      "Move to the next heading at the current level or higher.
    Skips all child headings."
      (interactive)
      (catch 'done
        (while t
          ;; 1. Try to go FORWARD to a sibling. If we succeed, we are done.
          (condition-case nil
              (progn (outline-forward-same-level 1) (throw 'done t))
            (error nil)) ;; Failed to move forward, loop continues
          ;; 2. Try to go UP to the parent. If we can't, we are at the top level.
          (condition-case nil
              (outline-up-heading 1)
            (error (throw 'done nil)))))) ;; Exit gracefully
    ;; Setting the leader map:
    (defvar-keymap rf-markdown-leader-map
      :doc "Keymap Markdown mode using a new leader key."
      :prefix 'rf-markdown-leader-map
      :repeat t
      "c" #'outline-previous-visible-heading
      "t" #'outline-next-visible-heading
      "h" #'outline-backward-same-level
    ...
    (keymap-set markdown-mode-map "<f6>" rf-markdown-leader-map)
    ;; Trick to use TAB -> markdown-cycle only in markdown-mode
    (defvar-keymap markdown-char-word-movement-repeat-map
      :doc "Keymap for repeating character and word movement in markdown-mode."
      :parent char-word-movement-repeat-map
      :repeat t
      "TAB" #'markdown-cycle))
...
#+begin_src emacs-lisp
  ...
  ;; Movements in parentheses in prog mode
  (keymap-set prog-mode-map "<f6> v" 'xah-goto-matching-bracket)
  (keymap-set prog-mode-map "<f6> m" 'xah-backward-left-bracket)
  (keymap-set prog-mode-map "<f6> w" 'xah-forward-right-bracket)
...
#+begin_src emacs-lisp
  ...
  (use-package treesit
    ...
    (use-package combobulate
      :vc
      (combobulate :url "https://github.com/mickeynp/combobulate" :rev :newest)
      ;; This part of the preface does not work when the package is installed
      ;; for the first time; it must be commented out to allow installation.
      :preface
      ...
      (defvar-keymap combobulate-navigate-repeat-map
        ...
        "l" #'combobulate-navigate-previous
        "s" #'combobulate-navigate-next)
...
#+begin_src emacs-lisp
  ...
  (use-package eglot
    ...
    ((vscode-langservers-extracted . "pn add -g vscode-langservers-extracted")
     (typescript-language-server . "pn add -g typescript-language-server"))
    :init
...
#+begin_src emacs-lisp
  ...
  (use-package flycheck
    ...
    (defun jb-use-oxlint-from-node-modules-flycheck ()
      (let* ((root (locate-dominating-file
                    (or (buffer-file-name) default-directory)
                    "node_modules"))
             (oxlint (and root
                          (expand-file-name "node_modules/oxlint/bin/oxlint"
                                            root))))
    ...
    :hook ((flycheck-mode . (lambda ()
                              ...
                              (jb-use-oxlint-from-node-modules-flycheck)
  ...
  ;; These programs must be installed separately (e.g, npm install eslint/oxlint)
...
#+begin_src emacs-lisp
  ...
  (use-package flymake-jsts
    :vc
    (flymake-jsts :url
                  "https://github.com/orzechowskid/flymake-jsts" :rev :newest)
    :defer t
    :init
        ...
        (when (and eslintx (file-executable-p eslintx))
          (setq-local flymake-jsts-executable-name-alist
                      `((eslint . ,eslintx)
                        (oxlint . "oxlint")
                        (biome . "biome")))))))
  ;; Make sure eslint/oxlint is installed and present on your emacs exec-path.
  ...
  (use-package flymake-stylelint
    :vc
    (flymake-stylelint
     :url "https://github.com/orzechowskid/flymake-stylelint"  :rev :newest)
    :defer t
    :init
...
#+begin_src emacs-lisp
  ...
  (use-package dape
    ...
    (defvar-keymap dape-prefix-map
      ...
      "j" #'dape-many-windows
      "q" #'dape-select-stack)
    (keymap-set prog-mode-map "<f6> q" dape-prefix-map)
...
#+begin_src emacs-lisp
  ...
  (use-package sgml-mode
    ...
    ;; ("M-i" . facemenu-keymap)
    :config
    ;; Remove combobulate binding that couse conflicts
...
#+begin_src emacs-lisp
  ...
  (use-package emacs
    :hook ((emacs-lisp-mode . electric-pair-local-mode)
           (lisp-interaction-mode . show-paren-local-mode))
...
#+begin_src emacs-lisp
  ...
  (use-package gptel
    ...
    :init (load "~/.emacs.d/lisp/gptel-directives-presets.el")
    :bind ("C-{" . rf-gptel)
    :config
    ;; Bindings:
    (defvar-keymap gptel-leader-map
      :doc "Keymap for gptel commands."
      "g" #'gptel
      "d" #'gptel-send   ;; or RET
      "u" #'gptel-menu
      "q" #'gptel-quick
      "r" #'gptel-rewrite
      "a" #'gptel-add
      "i" #'rf-gptel--inspect-fsm-interactive
      "b" #'gptel-abort
      "s" #'gptel-system-prompt
      "f" #'gptel-add-file
      "p" #'gptel-preset
      "t" #'gptel-tools)
    (keymap-set global-map "C-d" gptel-leader-map)
    ;; Default backend:
    (setq
    ...
    (gptel-make-openai "OpenRouter"
      ...
      :stream t
      :key gptel-api-key
      :models '(poolside/laguna-m.1:free ;; Programming 14
                openai/gpt-oss-120b:free ;; Programming 27
                nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free
                liquid/lfm-2.5-1.2b-thinking:free ;; Thinking
                qwen/qwen3-next-80b-a3b-instruct:free
                qwen/qwen3-coder:free
                anthropic/claude-opus-4.8-fast ;; Programming 48 Fast
                deepseek/deepseek-v4-flash ;; Programming 8
                xiaomi/mimo-v2.5 ;; Programming 1
                anthropic/claude-opus-4.7 ;; Programming 4
                tencent/hy3-preview)) ;; Programming 3 JS 1
    ...
    ;; Load the gptel MCP library:
    (require 'gptel-integrations)
    ;; Directives:
    (setf (alist-get 'english gptel-directives)
          "Correct the grammar, style, and naturalness of this statement. Just the correction, DO NOT explain it.")
    ;; Presets:
    ;; It differ from gptel-default in that it sets the system prompt
    (gptel-make-preset 'gptel-init
      :description "Use gptel's default intial configuration."
      :context nil :use-context 'system :tools nil :use-tools t
      :temperature nil :max-tokens nil :num-messages-to-send nil
      :request-params nil :org-convert-response t :track-media nil
      :track-response t :system 'default :stream t :cache nil)
    (gptel-make-preset 'add-english-checker
      :description "Direct assistant with speaker correction."
      :system (list :append rf-gptel-preset-direct-english-correction))
    ;; These are primarily to be used as a prefix (@key):
    (gptel-make-preset 'en   ;; @en glish
      :description "Correct English grammar and style only"
      :system rf-gptel-preset-correct-english
      :temperature 0.1
      :max-tokens 500)
    ;; Set model parameters from gptel-menu buffer-locally
    (setq gptel--set-buffer-locally t)
    ;; (gptel-preset 'add-english-checker)
    ;; Functions:
    (defun rf-gptel (&optional arg)
      "Switch to or create a new *gptel* chat session.
    With a nonnumeric prefix arg, create a new session."
      (interactive "P")
      (let ((buf (if arg
                     (gptel (generate-new-buffer "*gptel*"))
                   (save-excursion
                     (gptel "*gptel*")))))
        (switch-to-buffer buf)))
    (defun rf-gptel--inspect-fsm-interactive ()
      "Make the `gptel--inspect-fsm' function interactive."
      (interactive)
      (gptel--inspect-fsm))
    :custom
    ...
    :custom (mcp-hub-servers
             '(("filesystem" .
                (:command "npx"
                          ...
                          :roots ("/home/rich/projects/")))
               ("github" .
                (:command "npx"
                          :args ("-y" "@modelcontextprotocol/server-github")))
               ("fetch" . (:command "uvx" :args ("mcp-server-fetch")))))
    :config (require 'mcp-hub))
  ;; Start all MCPservers, such as launch all MCPservers after starting Emacs
  ;; Only if you want to always start all servers for all instances of emacs
  ;; :hook (after-init . mcp-hub-start-all-server)

#+end_src

*** Gptel agent
Agent mode for gptel. This is a collection of tools and prompts to use gptel “agentically” with any LLM, to autonomously perform tasks.
...
#+begin_src emacs-lisp
  ...
  (use-package gptel-quick
    ...
    :vc
    (gptel-quick :url
                 "https://github.com/karthink/gptel-quick" :rev :newest)
    :after (gptel embark)
    :config
    (keymap-set embark-general-map "?" #'gptel-quick))

  ;; Set here so that it is not overwritten when the package is loaded
  (with-eval-after-load 'gptel-quick
    (setq gptel-quick-display nil)
    ;; Qwen3.7-Plus:
    (defun my-inject-gptel-quick-copy-key (args)
      "Safely inject a remap for xah command in the gptel-quick transient map."
      (let ((map (car args)))
        ;; Verify this is gptel-quick map by checking its unique fingerprint
        ;; Ensures dont accidentally modify transient maps from other packages
        (when (and (keymapp map)
                   (lookup-key map [remap keyboard-quit])
                   (lookup-key map (kbd "+"))
                   (lookup-key map [remap kill-ring-save]))
          ;; Grab the internal copy-response function that gptel-quick created
          (let ((copy-cmd (lookup-key map [remap kill-ring-save])))
            ;; Add command to the map, pointing to that same internal function
            (define-key map [remap xah-copy-line-or-region-mod] copy-cmd))))
      ;; Return the arguments completely unchanged so `set-transient-map'
      args)
    ;; Apply the advice to the function that activates transient maps
    (advice-add 'set-transient-map
                :filter-args #'my-inject-gptel-quick-copy-key))
...
#+begin_src emacs-lisp
  ...
  (use-package agent-shell
    :ensure-system-package ((wl-paste . "apt install wl-clipboard")
                            (gemini . "pn add -g @google/gemini-cli"))
    ;; Enable 'agent-shell-buffers' to use 'rf-agent-shell-switch-buffer'
    :commands (agent-shell-buffers rf-agent-shell)
    :bind (("C-}" . rf-agent-shell)
           :map agent-shell-mode-map
           ("M-n" . nil) ("M-p" . nil) ("C-d" . nil) ("M-r" . nil)
           ("C-<up>" . nil) ("C-<down>" . nil)
           ("S-<return>" . nil) ("C-<tab>" . nil)
           ...
           ("M-n" . nil) ("M-p" . nil) ("M-r" . nil) ("C-<tab>" . nil)
           ...
           ("c" . diff-hunk-prev)
           ("t" . diff-hunk-next)
           :map acp-traffic-entry-mode-map
           ("c" . scroll-down-command)
           ("t" . scroll-up-command))
    :config
    ;; Bindings:
    (defvar-keymap agent-shell-leader-map
      :doc "Keymap for agent-shell commands."
      "b" #'agent-shell
    ...
    (keymap-set global-map "C-b" agent-shell-leader-map)
    ;; Configure authentication for the agent providers:
    (setq agent-shell-google-authentication
          (agent-shell-google-make-authentication
           :api-key (lambda () (auth-source-pass-get 'secret "api/gemini"))))
    ;; Enable the root dir as a trusted folder in Gemini
    (setq agent-shell-google-gemini-acp-command
          '("gemini" "--experimental-acp" "--skip-trust"))
    ;; Functions:
    (defun rf-agent-shell (&optional arg)
      "Switch to the most recently agent shell buffer.
    If there is no agent shell buffer, prompt to create one.
    If the current buffer is an agent and there are more agents,
    switch to another one.
    With C-u prefix ARG, force start a new shell.
    With C-u C-u prefix ARG, prompt to pick an existing shell."
      (interactive "P")
      (let ((buf (car (agent-shell-buffers)))
            (bufs (agent-shell-buffers)))
        (cond
         ((equal arg '(16))
         ...
         (t
          (if buf
              (if (and (eq (current-buffer) buf)
                ...
                (switch-to-buffer buf))
            ;; Change the following if you want a specific agent:
            (agent-shell--dwim))))))
    ;; Change the color of the cells that stand out:
    (custom-set-faces  '(agent-shell-markdown-table-zebra
                         ((t (:background "#2d3139"))))) ;; or #3e4451
    :custom
  ...
  ;; ;; Enable it if you want to use the `emacs-skills' or run M-x server-start
  ;; (server-start)
...
#+begin_src emacs-lisp
  ...
  ;; acp.el implements the Agent Client Protocol (ACP) for Emacs
  (use-package acp
    :bind (:map acp-traffic-mode-map
                ("n" . nil) ("p" . nil)
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'comint
    (keymap-set comint-mode-map "C-S-c" 'comint-previous-input)
    (keymap-set comint-mode-map "C-S-t" 'comint-next-input))
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'apropos
    (keymap-set apropos-mode-map "c" 'apropos-previous-symbol)
    (keymap-set apropos-mode-map "t" 'apropos-next-symbol))
