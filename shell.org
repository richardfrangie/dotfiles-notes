...
( ~gsettings get org.gnome.desktop.peripherals.mouse speed~ get the moue speed. -1 (slow) and 1 (fast) )
( ~gsettings set org.gnome.desktop.peripherals.mouse natural-scroll true~ set natural scrolling )
( ~gsettings get org.gnome.desktop.input-sources xkb-options~ input sources related various options for the X Keyboard Extension (XKB). Useful to Swap Control AltKeys )
...
( ~gsettings set org.gnome.desktop.a11y.mouse dwell-time 1.0~ set the dwell-time (delay) to 1.0 )
( ~gsettings set org.gnome.desktop.a11y.mouse dwell-threshold 1~ set the dwell-threshold (pixels movement allowed) to 1 )
( ~gsettings set org.gnome.desktop.peripherals.mouse left-handed true~ swap mouse buttons )
...
( ~ping 0~ command to ping the localhost )
...
( ~curl ipinfo.io/8.8.8.8~ get a clean JSON summary of any IP )
...
** pnpm
*** pnpm
Alias: pn is available as a short alias for pnpm.
[ /-C <path>, --dir <path>/ run as if pnpm was started in <path> instead of the current working directory /-w, --workspace-root/ run as if pnpm was started in the root of the workspace instead of the current working directory ]
( ~pn init~ create a package.json file (initialize a project) )
( ~pn init --bare~ creates a package.json with only the required fields )
( ~pn init --init-type <type>~ set the module system for the package )
( ~pn init --init-package-manager~ pin the project to the current pnpm version )

*** pnpm install
Is used to install all dependencies for a project.
Alias: i.
[ /-P/ if true, pnpm will not install any package listed in devDependencies and will remove those insofar they were already installed /-D/ only devDependencies are installed and dependencies are removed insofar they were already installed /--no-optional/ optionalDependencies are not installed /--force/
force reinstall dependencies: refetch packages modified in store /--offline/ if true, pnpm will use only packages already available in the store /--no-lockfile/ don't read or generate a pnpm-lock.yaml file /--lockfile-only/ when used, only updates pnpm-lock.yaml and package.json ]

*** pnpm add <pkg>
Installs a package (e.g. version: ^4.x.x) and any packages that it depends on. By default, any new package is installed as a production dependency.
[ /-D/ save to devDependencies /-O/ save to optionalDependencies /-g/ install package globally /-E/ saved dependencies will be configured with an exact version rather than using pnpm's default semver range operator ]
( ~pn add sax~ save to dependencies )
( ~pn add -D sax~ save to devDependencies )
( ~pn add -O sax~ save to optionalDependencies )
( ~pn add -g sax~  install package globally )
( ~pn add -g eslint prettier~ installing multiple packages in a single command groups them into *one isolated install* (CAUTION) )
( ~pn add sax@next~ install from the next tag )
( ~pn add sax@3.0.0~ specify version 3.0.0 )
( ~pn add ./some-directory~ install from the local file system )

*** pnpm update
Updates packages to their latest version based on the specified range.
Aliases: up, upgrade.
( ~pn up~ updates all dependencies, adhering to ranges specified in package.json )
( ~pn up --latest~ updates all dependencies to their latest versions )
( ~pn up foo@2~ updates foo to the latest version on v2 )
( ~pn up "@babel/*"~ updates all dependencies under the @babel scope )
( ~pn outdated~ checks for outdated packages )
( ~pn update -g [pkg]~ update global packages (re-installs into new isolated directories) )

*** pnpm remove
Removes packages from node_modules and from the project's package.json.
Aliases: rm, uninstall, un
[ /-g/ remove a global package /-D/ only remove the dependency from devDependencies /-O/ only remove the dependency from optionalDependencies /-P/ Only remove the dependency from dependencies ]
( ~pn rm -g sax~  remove a globally installed package (removes the entire installation group) )
( ~pn prune~ removes unnecessary packages )
( ~pn clean~ safely remove node_modules contents from all workspace projects (Alias: purge) )
( ~pn store prune~ removes unreferenced packages from the store )

*** pnpm list
Output all the versions of packages that are installed, as well as their dependencies, in a tree-structure.
Alias: ls
[ /-g/ list packages in the global install directory /--depth <number>/ max display depth of the dependency tree /--long/ show extended information /-P/ display only the dependency graph for packages in dependencies and optionalDependencies /-D/ display only the dependency graph for packages in devDependencies /--recursive, -r/ perform command on every package in subdirectories ]
( ~pn list -g~ list all globally installed packages )
( ~pn list -g --depth=<n>~ shows the full dependency tree )
( ~pn search <keyword>~ search the registry for packages matching the given keywords (Aliases: s, se, find) )
( ~pn view <pkg>~ view package metadata from the registry (Aliases: info, show) )
( ~pn repo lodash~ open the repo of a package on the registry )
( ~pn audit~ checks for known security issues with the installed packages )
( ~pn why <pkg>~ shows all packages that depend on the specified package )
( ~pn docs [<pkg> ...]~ open a package's documentation (or homepage) in the browser (Alias: home) )

*** pnpm run <script>
*Runs a script* defined in the package's manifest file.
In addition to the shell’s pre-existing PATH, pnpm run includes node_modules/.bin in the PATH provided to scripts. This means that so long as you have a package installed, you can use it in a script like a regular command. Even though the package is not installed globally in your shell, it will run.
( ~pn <script>~ shorthand (ONLY for scripts that do not share the same name as already existing pnpm commands) )
( ~pn run~ shows available scripts )
( ~pn set-script <name> <command>~ adds or updates an entry in the scripts field of the project manifest (Alias: ss) )
( ~pn set-script lint "eslint ."~ set the linter script )
( ~pn test~ runs an arbitrary command specified in the package's test property of its scripts object )
( ~pn start~ runs an arbitrary command specified in the package's start property of its scripts object. If no start property is specified on the scripts object, it will attempt to run node server.js as a default, failing if neither are present )
( ~pn run "/<regex>/"~ run multiple scripts at the same time by using a regex instead of the script name (note that regex is between slashes) )
( ~pn run "/^watch:.*/"~ run all scripts that start with watch: )
...
*Gemeni*: run tools that are already sitting inside your local node_modules.
( ~pn <cmd>~ shorthand (ONLY when the command is not in conflict with a builtin pnpm command ) )
( ~pn exec jest~ execute jest in scope of the project. If Jest is a dependency of the project, there is no need to install Jest globally )
( ~pn pm <cmd>~ always runs the built-in pnpm command, bypassing any same-named script in package.json. For example, if your project defines a "clean" script in package.json, then pnpm clean runs that script instead of the built-in pnpm clean )

*** pnx
*Fetches* a package from the registry without installing it as a dependency, hotloads it, and *runs* whatever default command binary it exposes.
Aliases: pnpm dlx, pnpx. (dlx = download execute)
*Gemeni*: check your local directory first, and downloads from the web second if it can't find it. It sandboxes execution and isolates the command. Because it lives up in that global vault, it operates inside its own little bubble, which is why it cannot see the local plugins.
[ /--package <name>/ the package to install before running the command ]
( ~pnx create-vue my-app~ use create-vue anywhere without needing to install it under another project. This will fetch create-vue from the registry and run it with the given arguments )
( ~pnx create-vue@next my-app~ create a project. Specify which exact version of the package )
( ~pnx npkill~ size of each of all packages in the current directory )
( ~pnx npm-check -u~ show updates of packages )

*** pnpm runtime <cmd>
Manage runtimes.
Alias: rt
[ /--global, -g/ globally ]
( ~pn runtime set <name> <version> [-g]~ install the specified version of a runtime )
( ~pn runtime set node 22 -g~ install Node.js v22 globally )
( ~pn runtime set node lts -g~ install the LTS version of Node.js )
( ~pn runtime set node latest -g~ install the latest version of Node.js )
( ~pn runtime set node argon -g~ pnpm runtime set node argon -g )

*** pnpm self-update [<version>]
Updates pnpm to the latest version or the one specified.
*Warning*: the behavior of pnpm self-update depends on the project context.
*In a project that pins pnpm*: When the project's package.json has a packageManager field set to pnpm (or a devEngines.packageManager entry for pnpm), self-update only updates the pinned version in package.json to the resolved one. It does not install pnpm globally. The next time you run a pnpm command, pnpm will automatically download and switch to the specified version.
*Outside a project (or when the pnpm pin is ignored)*: If the project does not pin pnpm, or the pin is being ignored via pmOnFail: ignore, self-update installs the resolved pnpm version globally and links it to PNPM_HOME so it becomes the active pnpm binary on your system.
*Warning*: depending on where it's run, it can cause version conflicts and lead to errors. If that happens, you'll have to reinstall it.
( ~pn self-update~ in the HOME directory updates globally )
( ~pn self-update 10~ update )
( ~pn self-update next-10~ update )
( ~pn self-update 10.6.5~ update )
( ~pn with <version|current> <args...>~ run pnpm at a specific version (or the currently running one) for a single invocation, ignoring the packageManager and devEngines.packageManager fields of the project's manifest )
( ~pn with current install~ run the globally installed pnpm, ignoring the version pinned in the manifest )
( ~pn with 11.0.0-rc.1 install~ run a specific version )
( ~pn version <newversion>~ bump the package version )
...
[ /--json/ when setting, parses each value as JSON before writing. When getting a single key, returns the JSON-encoded form instead of the raw value ]
( ~pn pkg get~ prints the full manifest (package.json) )
( ~pn pkg get name version~ retrieves these values from package.json )
( ~pn pkg set name=my-package~ sets one or more values in package.json )
( ~pn pkg get scripts.test~ retrieves this value from package.json )
( ~pn pkg set scripts.build="tsc -p ."~ sets one or more values in package.json )
( ~pn pkg set 'keywords[0]'=cli~ sets one or more values in package.json )
( ~pn pkg set private=true --json~  by default the value is stored as a string. Pass --json to parse the value as JSON before storing it (useful for booleans, numbers, arrays, and objects) )
( ~pn pkg delete scripts.test~  removes one or more keys from package.json )
( ~pn pkg fix~  auto-corrects common errors in package.json (e.g. removes a non-string name or version, drops dependency / scripts blocks whose values aren't objects, drops a bin field that's neither a string nor an object) )

*** pnpm config
Manage the configuration files.
Alias: c.
The global YAML configuration file (config.yaml) is located at: ~/.config/pnpm/config.yaml.
[ /-g/ set the configuration in the global config file /--json/ make get and list show all the config settings in JSON format and make set parse the value as JSON ]
( ~pn c list~ show all the config settings. Output is a JSON object. Auth-related settings are hidden from the output; use pnpm config get <key> to read them explicitly )
( ~pn c set <key> <value>~ set the config key to the value provided )
( ~pn c set --location=project nodeVersion 22.0.0~ set config )
( ~pn c get <key>~ print the config value for the provided key )
( ~pn c get nodeVersion~ print the node version )
( ~pn c get --json nodeVersion~ print the node version in json )
( ~pn c delete <key>~ remove the config key from the config file )
( ~pn c set fetch-timeout 300000~ temporarily increase the network timeout. This is useful when problems arise during node version installation )

*** pnpm doctor
Checks for known common issues with pnpm configuration.
( ~pn help -a~ print all the available commands )
( ~pn help <cmd>~ display help information about pnpm )
( ~pn ping~ ping the configured registry to verify connectivity. This is useful for quickly confirming that the current machine can reach the registry without installing or publishing anything )
...
( ~npm outdated --global~ check for outdated global packages )
...
( ~uv tool run~ same as above )
( ~uvx ruff~ run ruff )
( ~uvx pycowsay hello from uv~ run pycowsay with arguments )
...
[ /-resize 30%/ resize /-quality 20%/ quality ]
...
( ~optipng file.png~ compress and optimization )
( ~optipng *.png~ optimize the all .png files in a folder )

*** pngquant -Q 65-80 image.png
...
** Oxlint
*** oxlint [OPTIONS] [PATH]
Run the linter. If PATH is omitted, Oxlint lints the current working directory.
[ /-c/ oxlint configuration file /--fix/ fix as many issues as possible /--fix-suggestions/ apply auto-fixable suggestions (may change program behavior) /--fix-dangerously/ apply dangerous fixes and suggestions ]
( ~oxlint -c ./oxlintrc.json~ pass a config explicitly )

*** oxlint ---fix
Apply safe fixes.
( ~oxlint --fix-suggestions~ apply suggestions (may change program behavior) )
( ~oxlint --fix-dangerously~ apply dangerous fixes and suggestions )

*** oxlint --init
Initialize the .oxlintrc.json config with default values.

*** oxlint --rules
List registered rules, including those enabled by the config.

*** oxlint --ignore-path .oxlintignore
Use an explicit ignore file.
( ~oxlint --ignore-pattern "dist/**" --ignore-pattern "*.min.js"~ add ignore patterns from the command line )
( ~oxlint --no-ignore~ disable ignore handling )

*** oxlint -D no-alert
Override severity from the CLI.
Arguments are applied from left to right.
[ /-A/ allow /-W/ warn /-D/ deny ]
( ~oxlint -D no-alert -W oxc/approx-constant -A no-plusplus~ several severities )
( ~oxlint -D correctness -D suspicious~ change categories )

** Stylelint
*** stylelint "**/*.css"
Run Stylelint on all the CSS files in your project (note the **/*asterisks).
[ /--fix/ automatically fix as many problems as possible /--cache/ option to improve performance /--formatter/ option to customize the output ]
( ~stylelint "**/*.css" --fix~ fix as many problems as possible )
( ~stylelint "foo/**/*.css"~ recursively linting all .css files in the foo directory )
( ~stylelint "**/*.{css,scss,sass}"~ linting all .css, .scss, and .sass files )
( ~echo "a { color: pink; }" | stylelint~ linting stdin )
( ~stylelint "**/*.css" "!**/docker/**"~ linting all .css files except those within docker subfolders, using negation in the input glob )
...
( ~stylelint "foo/**/*.scss" --cache --cache-location "/Users/user/.stylelintcache/"~ caching processed .scss files foo directory )
...
( ~stylelint "foo/**/*.css" --custom-syntax path/to/my-custom-syntax.js~ recursively linting all .css files in the foo directory using a custom syntax )
( ~stylelint -f json "*.css" 2>&1 | jq '[.[] | .warnings | length] | add'~ use a report as input for another command through piping )
...
Setup Stylelint and extend our standard config.
( ~pnpm create stylelint@latest~ the latest version )

** Oxfmt
*** oxfmt [OPTIONS] [PATH]
Running oxfmt without arguments formats the current directory.
...
[ /-c/ path to the configuration file /-init/ initialize /--check/ check if files are formatted, also show statistics /--list-different/ list files that would be changed /-V/ version ]
( ~oxfmt '!**/fixtures/*.js'~ exclude patterns with ! prefix )

*** oxfmt --check
Check if files are formatted, also show statistics.

*** oxfmt --init
Initialize/create .oxfmtrc.json config file with defaults.

*** oxfmt --list-different
Lists all files that need formatting, without actually changing them.

** Servers
*** vite
Start Vite dev server in the current directory.
The index.html will be served on http://localhost:5173.
( ~vite src/ --port 1234~ start the server )

*** servor <root> <fallback> <port>
Run as a terminal command without adding it as a dependency.
The index.html will be served on http://127.0.0.1:8080.
[ /---reload/ causes the browser to reload when files change /--browse/ causes the browser to open when the server starts ]
( ~servor . main.html 8383 --reload~ start the server )

*** live-server <path-to-content>
...
[ /-c/ use specified config file /--host [host]/ specify hostname /--port <port>/ specify port /--force/ force the optimizer to ignore the cache and re-bundle ]
( ~vite src/ --port 1234~ start the server )
...
( ~pnpm create vite@latest~ scaffold a vite project )
( ~pnpm create vite my-app -- --template vanilla~ scaffold a Vite + Vanilla project )
( ~pnpm create vite my-vue-app --template vue --no-interactive~ create a project without interactive prompts )
...
** keyd
*** keyd.rvaiya list-keys
List valid key names.

*** keyd.rvaiya monitor
Get key names/device ids.
...
*** keyd bind '- = C=c'
Apply the supplied binding.

*** man keyd.rvaiya
Man page.

** pass
*** pass init name@mail.com
Initialize password store.
[ /-n/ no symbols /-c/ copy it to the clipboard instead of displaying it at the console ]

*** pass ls
List all the existing passwords in the store.
( ~pass Folder/file~ show existing password )
( ~pass find .com~ find existing passwords in store that match .com )

*** pass insert Folder/file
Add password to store.
( ~pass insert -m Folder/file~ add multiline password to store )

*** pass show Folder/file
Decrypt  and print a password named pass-name.
( ~pass Folder/file~ same as above )

*** pass -c Folder/file
Copy existing password to clipboard.

*** pass generate Folder/file 15
Generate new passwords using /dev/urandom internally.
( ~pass generate -n Folder/file 15~ generate new alphanumeric password )
( ~pass generate -c Folder/file 15~ generate new password and copy it to the clipboard )

*** pass rm Folder/file
Remove password.

*** pass edit name
Edited in the default text editor.
...
List of possibilities key values.
...
*** logkeys --start
Start logging to default log file.

*** logkeys --start --device event11
Use a custom event device, e. g. wireless keyboard.

*** logkeys --start --keymap=file.map --device event11
Use file.map as input keymap for processing pressed keys.

*** logkeys --kill
End running logkeys process.

*** logkeys --no-timestamps
Doesn't prepend timestamp to each line of log file.
...
*** git clone <url>
Get a copy (create a local copy) of an existing Git repository.
...
( ~git add .~ stage all (new, modified, but NOT deleted) files in current dir and subdirs (not higher directories) )
( ~git add -A~ stage *all* (new, modified, deleted) files )
( ~git add -u~ (update) stage modified and deleted  of *already-tracked* files only (*not new files*) )
...
( ~git branch -vv~ shows all your local branches with their upstream tracking info and commit status )
...
Moves (by force) the main branch to three parents behind HEAD (reassign a branch to a commit).
...
*** git checkout main^
Moving upwards one commit from /main/.
...
( ~git checkout HEAD~3~ moving upwards three times )
( ~git checkout main^^~ moving upwards to the grandparent )
...
*** git reset
We use when we want to move the repository back to a previous commit, discarding any changes made after that commit. "*rewriting history*;" git reset will move a branch backwards as if the commit had never been made in the first place.
...
git fetch performs two main steps, and two main steps only. It: downloads the commits that the remote has but are missing from our local repository, and... updates where our remote branches point (for instance, origin/main) (you can think of running git fetch as a *download* step)
...
[ /--verify-signatures/  inspect and reject when merging a commit that does not carry a trusted GPG signature ]
( ~git fetch ; git merge FETCH_HEAD~ equivalent command )
( ~git pull --rebase~ shorthand for a fetch and a rebase )
...
( ~gpg --list-secret-keys~ list  the specified secret keys )
( ~gpg --list-key keyID~ show the public key currently stored )

*** gpg --full-generate-key
Generate a GPG key pair.
( ~gpg --quick-generate-key "Comment"~ simple command to generate a standard key (primary key and subkey are created) )
( ~gpg --quick-generate-key "Comment" rsa4096~ simple command to generate a standard key with rsa algorithm (only the primary key is created) )
