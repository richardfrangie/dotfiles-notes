...
( ~pnpm init~ create a package.json file (initialize a project) )
( ~pnpm init --bare~ creates a package.json with only the required fields )
( ~pnpm init --init-type <type>~ set the module system for the package )
( ~pnpm init --init-package-manager~ pin the project to the current pnpm version )
...
( ~pnpm add -g eslint prettier~ installing multiple packages in a single command groups them into *one isolated install* (CAUTION) )
( ~pnpm add sax@next~ install from the next tag )
( ~pnpm add sax@3.0.0~ specify version 3.0.0 )
( ~pnpm add ./some-directory~ install from the local file system )
...
( ~pnpm rm -g sax~  remove a globally installed package (removes the entire installation group) )
( ~pnpm prune~ removes unnecessary packages )
( ~pnpm clean~ safely remove node_modules contents from all workspace projects (Alias: clean) )
...
( ~pnpm list -g --depth=<n>~ shows the full dependency tree )
( ~pnpm search <keyword>~ search the registry for packages matching the given keywords (Aliases: s, se, find) )
( ~pnpm view <pkg>~ view package metadata from the registry (Aliases: info, show) )
( ~pnpm repo lodash~ open the repo of a package on the registry )
( ~pnpm audit~ checks for known security issues with the installed packages )
( ~pnpm why <pkg>~ shows all packages that depend on the specified package )
( ~pnpm docs [<pkg> ...]~ open a package's documentation (or homepage) in the browser (Alias: home) )
...
( ~pnpm <script>~ shorthand (ONLY for scripts that do not share the same name as already existing pnpm commands) )
( ~pnpm run~ shows available scripts )
( ~pnpm set-script <name> <command>~ adds or updates an entry in the scripts field of the project manifest (Alias: ss) )
( ~pnpm set-script lint "eslint ."~ set the linter script )
( ~pnpm test~ runs an arbitrary command specified in the package's test property of its scripts object )
( ~pnpm start~ runs an arbitrary command specified in the package's start property of its scripts object. If no start property is specified on the scripts object, it will attempt to run node server.js as a default, failing if neither are present )
( ~pnpm run "/<regex>/"~ run multiple scripts at the same time by using a regex instead of the script name (note that regex is between slashes) )
( ~pnpm run "/^watch:.*/"~ run all scripts that start with watch: )
...
( ~pnpm <cmd>~ shorthand (ONLY when the command is not in conflict with a builtin pnpm command ) )
( ~pnpm exec jest~ execute jest in scope of the project. If Jest is a dependency of the project, there is no need to install Jest globally )
( ~pnpm pm <cmd>~ always runs the built-in pnpm command, bypassing any same-named script in package.json. For example, if your project defines a "clean" script in package.json, then pnpm clean runs that script instead of the built-in pnpm clean )
...
( ~pnpx npkill~ size of each of all packages in the current directory )
( ~pnpx npm-check -u~ show updates of packages )

*** pnpm runtime <cmd>
Manage runtimes.
Alias: rt
[ /--global, -g/ globally ]
( ~pnpm runtime set <name> <version> [-g]~ install the specified version of a runtime )
( ~pnpm runtime set node 22 -g~ install Node.js v22 globally )
( ~pnpm runtime set node lts -g~ install the LTS version of Node.js )
( ~pnpm runtime set node latest -g~ install the latest version of Node.js )
( ~pnpm runtime set node argon -g~ pnpm runtime set node argon -g )
...
( ~pnpm self-update~ in the HOME directory updates globally )
( ~pnpm self-update 10~ update )
( ~pnpm self-update next-10~ update )
( ~pnpm self-update 10.6.5~ update )
( ~pnpm with <version|current> <args...>~ run pnpm at a specific version (or the currently running one) for a single invocation, ignoring the packageManager and devEngines.packageManager fields of the project's manifest )
( ~pnpm with current install~ run the globally installed pnpm, ignoring the version pinned in the manifest )
( ~pnpm with 11.0.0-rc.1 install~ run a specific version )
( ~pnpm version <newversion>~ bump the package version )
...
( ~pnpm pkg get~ prints the full manifest (package.json) )
( ~pnpm pkg get name version~ retrieves these values from package.json )
( ~pnpm pkg set name=my-package~ sets one or more values in package.json )
( ~pnpm pkg get scripts.test~ retrieves this value from package.json )
( ~pnpm pkg set scripts.build="tsc -p ."~ sets one or more values in package.json )
( ~pnpm pkg set 'keywords[0]'=cli~ sets one or more values in package.json )
( ~pnpm pkg set private=true --json~  by default the value is stored as a string. Pass --json to parse the value as JSON before storing it (useful for booleans, numbers, arrays, and objects) )
( ~pnpm pkg delete scripts.test~  removes one or more keys from package.json )
( ~pnpm pkg fix~  auto-corrects common errors in package.json (e.g. removes a non-string name or version, drops dependency / scripts blocks whose values aren't objects, drops a bin field that's neither a string nor an object) )
...
( ~pnpm help -a~ print all the available commands )
( ~pnpm help <cmd>~ display help information about pnpm )
( ~pnpm ping~ ping the configured registry to verify connectivity. This is useful for quickly confirming that the current machine can reach the registry without installing or publishing anything )
...
( ~pngquant -f --ext .png -Q 65-80 image.png~ overwrites the original images )
