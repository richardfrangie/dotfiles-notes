...
*Note (GPT-5 mini)*: CSS custom property for ~::before/::after~ must include quotes when used as content; set it like (or include the quotes in the value you store):

#+begin_src javascript

  el.style.setProperty('--rank-suit', `"${this.rank}${this.suit}"`);
